...
else
{
  ...
  if (isset($_POST['add_transfer_fee'])) {
    ...
    if ($name != "") {
      ...
      $select_t_fee_query = mysqli_query($conn,"SELECT * FROM transfer_fee where home_no='$home_no'");
  		if(mysqli_num_rows($select_t_fee_query)>0)
  		{
  		  echo "<script> alert('ટ્રાન્સફર ફી અગાઉ ભરાય ગયેલ છે , તેથી કૃપા કરીને માહિતી ચકાસો.') </script>";
  		  echo "<script type='text/javascript'> document.location = 'add_transfer_fee.php'; </script>";
  		  exit;
  		}
      else {

        $add_transfer_fee_query = mysqli_query($conn, "INSERT INTO `transfer_fee`(`name`, `mobile_no`, `section`, `home_no`, `line_no`, `amount`, `payment_date`) VALUES ('$name','$mobile_no','$section','$home_no','$line_no','$amount','$date')");
        if ($add_transfer_fee_query) {
                      ...
                      <div class="form-row">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <input list="no" class="form-control" name="line_no" Placeholder="select">
                                <datalist id="no">
                                  <option value="1">
                                  <option value="2">
                                  <option value="3">
                        ...
                        <button class="btn btn-primary" type="submit" name="add_transfer_fee">Add User</button>
                    </form>
                  </div>
              </div>
            </div>
