...
else
{
  ...
  if (isset($_POST['add_devashibhai'])) {
    ...
    if ($description != "") {
      ...
      $select_devashibhai_query = mysqli_query($conn,"SELECT * FROM devashibhai where description='$description' AND amount='$amount'");
  		if(mysqli_num_rows($select_devashibhai_query)>0)
  		{
  		  echo "<script> alert('આ રકમ અગાઉ દેવશીભાઈ ના ખાતામાં લખાય ગયોલી છે, તેથી કૃપા કરીને માહિતી ચકાસો.') </script>";
  		  echo "<script type='text/javascript'> document.location = 'add_devashibhai.php'; </script>";
  		  exit;
  		}
      else {

        $add_devashibhai_query = mysqli_query($conn, "INSERT INTO `devashibhai`(`name`,`amount`, `amount_date`, `description`) VALUES ('$name','$amount','$date','$description')");
        if ($add_devashibhai_query) {
                  ...
                  <div class="card-body pr-2 pl-2">
                    <form  method="post">
                      <div class="form-row">
                        ...
                        <button class="btn btn-primary" type="submit" name="add_devashibhai">Add User</button>
                    </form>
                  </div>
              </div>
            </div>
