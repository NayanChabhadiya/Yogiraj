...
else
{
  ...
  if (isset($_POST['add_upad'])) {
    ...
    if ($description != "") {
      ...
      $select_upad_query = mysqli_query($conn,"SELECT * FROM upad where description='$description' AND amount='$amount'");
  		if(mysqli_num_rows($select_upad_query)>0)
  		{
  		  echo "<script> alert('ઉપાડ અગાઉ લખાય ગયેલો છે, તેથી કૃપા કરીને માહિતી ચકાસો.') </script>";
  		  echo "<script type='text/javascript'> document.location = 'add_upad.php'; </script>";
  		  exit;
  		}
      else {

        $add_upad_query = mysqli_query($conn, "INSERT INTO `upad`(`name`,`amount`, `upad_date`, `description`) VALUES ('$name','$amount','$date','$description')");
        if ($add_upad_query) {
                  ...
                  <div class="card-body pr-2 pl-2">
                    <form  method="post">
                      <div class="form-row">
                        ...
                        <button class="btn btn-primary" type="submit" name="add_upad">Add User</button>
                    </form>
                  </div>
              </div>
            </div>
