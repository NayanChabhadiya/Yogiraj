...
else
{
  ...
  if (isset($_POST['add_expance'])) {
    ...
    if ($description != "") {
      ...
      $select_expance_query = mysqli_query($conn,"SELECT * FROM 'expance' WHERE 'description'='$description' AND 'amount'='$amount' AND 'date'='$date'");
  		if(mysqli_num_rows($select_expance_query)>0)
  		{
  		  echo "<script> alert('ખર્ચ અગાઉ લખાય ગયેલો છે, તેથી કૃપા કરીને માહિતી ચકાસો.') </script>";
  		  echo "<script type='text/javascript'> document.location = 'add_expance.php'; </script>";
  		  exit;
  		}
      else {

        $add_expance_query = mysqli_query($conn, "INSERT INTO `expance`(`name`,`amount`, `date`, `description`) VALUES ('$name','$amount','$date','$description')");
        if ($add_expance_query) {
                  ...
                  <div class="card-body pr-2 pl-2">
                    <form  method="post">
                      <div class="form-row">
                        ...
                        <button class="btn btn-primary" type="submit" name="add_expance">Add User</button>
                    </form>
                  </div>
              </div>
            </div>
