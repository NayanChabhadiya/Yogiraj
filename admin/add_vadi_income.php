...
else
{
  ...
  if (isset($_POST['add_vadi_income'])) {
    ...
    if ($description != "") {
      ...
      $select_vadi_income_query = mysqli_query($conn,"SELECT * FROM vadi_income where description='$description' AND amount='$amount'");
  		if(mysqli_num_rows($select_vadi_income_query)>0)
  		{
  		  echo "<script> alert('આ પ્રસંગનું ભાડુ અગાઉ ચુકવાય ગયેલુ છે , તેથી કૃપા કરીને માહિતી ચકાસો.') </script>";
  		  echo "<script type='text/javascript'> document.location = 'add_vadi_income.php'; </script>";
  		  exit;
  		}
      else {

        $add_vadi_income_query = mysqli_query($conn, "INSERT INTO `vadi_income`(`party_name`, `address`, `function_name`, `s_name`, `function_date`, `function_bhadu`, `lightbill`, `table_bhadu`, `chair_bhadu`, `gt_bhadu`, `mixer_bhadu`, `gesbill`, `charge`, `total_amount`, `payment_date`, `sleep_no`, `description`) VALUES ('$party_name', '$address', '$function_name', '$s_name', '$function_date', '$function_bhadu', '$lightbill', '$table_bhadu', '$chair_bhadu', '$gt_bhadu', '$mixer_bhadu', '$gesbill', '$charge', '$total_amount', '$payment_date', '$sleep_no', '$description')");
        if ($add_vadi_income_query) {
                      ...
                      <h4 class="text-primary mb-0">ભાડાની રકમ</h4></br>
                        <div class="form-row">
                          <div class="form-group col-md-1">
                            ...
                            <input type="text" class="form-control" name="function_bhadu" required>
                          </div>
                          <div class="form-group col-md-1">
                            <label>લાઈટબિલ</label>
                            <input type="text" class="form-control" name="lightbill" required>
                          </div>
                          <div class="form-group col-md-2">
                            <label>ટેબલ ભાડુ (૫૦ રુ. લેખે )</label>
                            <input type="text" class="form-control" name="table_bhadu">
                          </div>
                          <div class="form-group col-md-2">
                            <label>ખુરશી ભાડુ (૭ રુ. લેખે )</label>
                            <input type="text" class="form-control" name="chair_bhadu">
                          </div>
                          <div class="form-group col-md-2">
                            <label>ગાદલા-ટકિયા ભાડુ (૨૫ રુ. લેખે )</label>
                            <input type="text" class="form-control" name="gt_bhadu">
                          </div>
                          <div class="form-group col-md-2">
                            <label>મિક્સર ભાડુ (૧૦૦ રુ. લેખે )</label>
                            <input type="text" class="form-control" name="mixer_bhadu">
                          </div>
                          <div class="form-group col-md-2">
                            <label>ગેસ બિલ (૭૦ રુ. લેખે )</label>
                            <input type="text" class="form-control" name="gesbill">
                          </div>
                        </div>
                        <div class="form-row">
                          <div class="form-group col-md-2">
                          ...
                          <div class="form-group col-md-2">
                            ...
                            <input type="text" class="form-control" name="sleep_no">
                          </div>
                        </div>
                        <div class="form-row">
                        ...
                        <button class="btn btn-primary" type="submit" name="add_vadi_income">Add User</button>
                    </form>
                  </div>
              </div>
            </div>
