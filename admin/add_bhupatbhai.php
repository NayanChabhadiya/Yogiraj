...
else
{
  ...
  if (isset($_POST['add_bhupatbhai'])) {
    ...
    if ($description != "") {
      ...
      $select_bhupatbhai_query = mysqli_query($conn,"SELECT * FROM bhupatbhai where description='$description' AND amount='$amount' AND name='$name'");
  		if(mysqli_num_rows($select_bhupatbhai_query)>0)
  		{
  		  echo "<script> alert('આ રકમ અગાઉ ભુપતભાઈ ના ખાતામાં લખાય ગયોલી છે, તેથી કૃપા કરીને માહિતી ચકાસો.') </script>";
  		  echo "<script type='text/javascript'> document.location = 'add_bhupatbhai.php'; </script>";
  		  exit;
  		}
      else {

        $add_bhupatbhai_query = mysqli_query($conn, "INSERT INTO `bhupatbhai`(`name`,`amount`, `amount_date`, `description`) VALUES ('$name','$amount','$date','$description')");
        if ($add_bhupatbhai_query) {
                  ...
                  <div class="card-body pr-2 pl-2">
                    <form  method="post">
                      <div class="form-row">
                        ...
                        <button class="btn btn-primary" type="submit" name="add_bhupatbhai">Add User</button>
                    </form>
                  </div>
              </div>
            </div>
